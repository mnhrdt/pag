...
%		(\R/2\pi\R)^n=\T^n}\\
		...
		\solution{\left\{\left(\begin{smallmatrix}d_1&&\\&\ddots&\\0&&
			\Pi_{i=1}^{n-1}d_i^{-1}\end{smallmatrix}\right)\right\}
		}
		\\
		\TS_n(\R) \cap \SO_n(\R) &=
		\solution{\left\{\left(\begin{smallmatrix}\pm 1&&0\\&\ddots&\\0&&\pm
...
(on le vérifie cette propriété d'abord pour les monomes, puis on l'étend à
tous les polynômes par linearité).  On a
donc~$P(\lambda)\in\mathrm{sp}_\C(P(A))$.\\
	\fbox{$\ds \mathrm{sp}_\C(P(A))\subseteq P(\mathrm{sp}_\C(A))$}
	Ce résultat dépend du théorème fondamental de l'algèbre.  Pour un contre
example sur~$\R$, considérez une matrice de rotation de~$90^\circ$, qui n'a
pas de valeurs propres réelles, mais la matrice au carrée a une valeur
propre~$-1$.  Pour démontrer le résultat sur~$\C$,
soit~$\lambda\in\mathrm{sp}_\C(P(A))$.  Par le théorème fondamental de
l'algèbre, le polynôme~$P(X)-\lambda$ peut se factoriser de la
forme~$P(X)-\lambda=a\Pi_n(X-b_n)$ avec~$a,b_n\in\C$.  On a
...
le sera pas non plus.  On a donc un vecteur~$x$ tel que~$P(A)x-\lambda x=0$
et
}
