...
\begin{exercice}
	...
	\end{enumerate}
\end{exercice}
\solution{\sl(Le fait que la norme induite est une norme, et les deux
	propriétés ci-dessus, suit directement des définitions.)
}
...
\end{exercice}
\solution{
	C'est la norme euclidienne dans l'espace vectoriel~$M_{n,n}(\K)=\K^{n^2}$.
	Il reste à vérifier la propriété~$\ABS{AB}_F\le\ABS{A}_F\ABS{B}_F$.
	Soient donc~$A=(a_{ij})$, $B=(b_{ij})$, $C=(c_{ij})$ avec $C=AB$.
	On a
	\(
		\ds c_{ij}=\sum_k a_{ik}b_{kj}
	\)
	et alors
	\[\small
		\ABS{C}^2
		= \sum_{ij} \Abs{c_{ij}}^2
		= \sum_{ij} \Abs{\sum_k a_{ik}b_{kj}}^2
		\le \sum_{ij}\left(\sum_k\Abs{a_{ik}}^2\sum_k\Abs{b_{kj}}^2\right)
		= \left(\sum_{ik}\Abs{a_{ik}}^2\right)
		\left(\sum_{jk}\Abs{b_{kj}}^2\right)
		=\ABS{A}^2\cdot\ABS{B}^2
	\]
	où l'inégalité est l'inégalité de Cauchy-Schwartz appliquée sur les
	vecteurs~$a_{i*}$ et~$b_{*j}$.  Finalement,~$\ABS{\cdot}_F$ n'est pas une
	norme induite pour~$n>1$ car~$\ABS{I}_F=\sqrt{n}$.
}
...
\solution{
	\fbox{$\ABS{\cdot}_2$}
	Par définition de norme induite:
	\[
		\ABS{A}^2_2
		=
		\left(
			\max_{\Abs{\x}_2\neq 0}\frac{\Abs{A\x}_2}{\Abs{\x}_2}
		\right)^2
		=
		\left(
		...
		\right)^2
	\]
	et on reconnait dans ce dernier terme le quotient de Rayleigh de la matrice
	hermitienne~$A^*A$ qui prend son maximum sur~$\rho(A^*A)$
	\fbox{$\ABS{\cdot}_2$}
}
...
\end{exercice}
\solution{
	Soit~$A\in\GL_n$ et soient~$x$, $b$, $\delta x$ et~$\delta b$ tels
	que~$Ax=b$ et $A(x+\delta x)=b+\delta b$.  On a,
	\[
		\mathrm{cond}(A)
		=
		\max_{p\neq 0}
		...
		\ge
		\frac{\Abs{Ax}}{\Abs{x}}
		\frac{\Abs{\delta x}}{\Abs{A\delta x}}
		=
		\frac{b}{\Abs{x}}
		\frac{\Abs{\delta x}}{\Abs{\delta b}}
	\]
	l'égalité a lieu lorsque les deux maximums sont atteints, ce qui arrive
	toujours car la sphère unité est compacte.
	Par exemple, si~$A\in\SSS^+_n(\R)$ on peut choisir~$x$ le vecteur propre de
	valeur propre plus grande de~$A$, et~$\delta x$ celui de valeur propre plus
	petite.
}
...
\end{exercice}
\solution{
	Soit~$A\in\GL_n$ et~$x$, $b$, $\delta x$ et~$\delta A$ tels
	que~$Ax=b$ et $(A+\delta A)(x+\delta x)=b$.  Ces conditions
	impliquent~$(\delta A)(x+\delta x)=-A(\delta x)$ et par définition
	de~$\mathrm{cond}(A)$ on a:
	\[
		\mathrm{cond}(A)
		\cdot\frac{\Abs{\delta A}}{\Abs{A}}
		=
		\ABS{A^{-1}}\cdot\ABS{\delta A}
		=
		\max_{p\neq 0}
		\frac{\Abs{p}}{\Abs{Ap}}
		\max_{q\neq 0}
		\frac{\Abs{\delta A q}}{\Abs{q}}
		\ge
		\frac{\Abs{\delta x}}{\Abs{A\delta x}}
		\frac{\Abs{\delta A (x+\delta x)}}{\Abs{x+\delta x}}
		=
		\frac{\Abs{\delta x}}{\Abs{x+\delta x}}
	\]
	d'où le résultat.  Par le même argument que avant (compacité de la sphère
	unité) cette inégalité est optimale.
}
...
\begin{proposition}[rotation de Yaroslavsky]
