...
$\quad $ {\color{black}1.3. Normes matricielles et conditionnement}\\
...
2. Trois caractérisations de~$\mathrm{sp}(A)$: alg., géo., optim.

3. Théorèmes de Gershgorin: $\mathrm{sp}(A)\subseteq\mathcal{G}(A),\ldots$

4. Décomposition polaire: $\GL_n(\C)=\HH_n^{++}(\C)\times\UU_n(\C)$

5. Décomposition SVD: $\M_{p,q}(\C)\approx \UU_p(\C)\times
\mathrm{diag}^+_{p,q}(\R) \times \UU_q(\C)$

6. Pseudo-inverse: $Ax=b$ résolu par~$x=A^\dagger b$, avec~$A^+$
